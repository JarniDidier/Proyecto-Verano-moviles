...
export class ErrorsService {
  ...
  private readonly messages = {
    ...
    range: 'La longitud del texto no está dentro del rango permitido.',
  ...
  public msg(key: 'between' | 'range', min: number, max: number): string; // <-- 'range'
  ...
  public msg(key: ErrorKey, a?: number | string, b?: number): string {
    ...
    switch (key) {
      ...
      case 'range':
        return `Debe tener entre ${a} y ${b} caracteres.`;
      case 'max':
