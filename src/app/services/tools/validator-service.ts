...
export class ValidatorService {
  ...
  // 1) Validar ID de Usuario (Exactamente 8 caracteres alfanuméricos)
  public idUsuario(val: string): boolean {
    if (!val) return false;
    return /^[a-zA-Z0-9]{8}$/.test(val);
  }

  // 2) Validar CURP (18 caracteres alfanuméricos en mayúsculas)
  public curp(val: string): boolean {
    if (!val) return false;
    return /^[A-Z0-9]{18}$/.test(val);
  }

  // 3) Validar RFC (12 o 13 caracteres alfanuméricos en mayúsculas)
  public rfc(val: string): boolean {
    if (!val) return false;
    return /^[A-Z0-9]{12,13}$/.test(val);
  }
